package app.core.connectivity.data.di

import app.core.connectivity.data.WearNodeDiscovery
import app.core.connectivity.data.messaging.WearMessagingClient
import app.core.connectivity.domain.NodeDiscovery
import app.core.connectivity.domain.messaging.MessagingClient
import org.koin.core.module.dsl.singleOf
import org.koin.dsl.bind
import org.koin.dsl.module

val coreConnectivityDataModule = module {
    singleOf(::WearNodeDiscovery).bind<NodeDiscovery>()
    singleOf(::WearMessagingClient).bind<MessagingClient>()
}